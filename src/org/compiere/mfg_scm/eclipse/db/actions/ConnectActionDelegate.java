...
public class ConnectActionDelegate implements IWorkbenchWindowActionDelegate {
	/*
	...
	public void run(IAction action) {
		if (DbfLauncherPlugin.checkDbfSettingsAndWarn()) {
			DbfLauncherPlugin.log(DbfLauncherPlugin
					.getResourceString("msg.connect"));
			try {
				DbfLauncherPlugin.getDefault().getDbfBootstrap().connect();
			} catch (Exception ex) {
				String msg = DbfLauncherPlugin
						.getResourceString("msg.connect.failed");
